...
def main():
    ...
    if len(lm_results):
        for result in lm_results:
          if result != "":
            #print(result)
            with open('./fliplogs/logs_f1.txt', 'a', encoding="utf-8") as fAp2:
              if len(result) == 7:
              ...
              fAp2.write(toprint)
              #print(toprint)
            with open('./fliplogs/logs_f2.txt', 'a', encoding="utf-8") as fAp3:
                  if len(result) == 7:
                    toprint = "\n" + "/viewauction " + str(result[0]) + " | Item: `" + str(result[1]) + "` | Price: `{:,}`".format(result[2]) + " | Second LBIN: `{:,}`".format(result[5]) + " | Clean LBIN `{:,}`".format(result[6])
                  else:
                    toprint = "\n" + "/viewauction " + str(result[0]) + " | Item: `" + str(result[1]) + "` | Price: `{:,}`".format(result[2]) + " | Second LBIN: `{:,}`".format(result[4])
                  fAp3.write(toprint)
            with open('./fliplogs/logs_f2_2.txt', 'a', encoding="utf-8") as fAp3_2:
            ...
            with open('./fliplogs/logs_f3.txt', 'a', encoding="utf-8") as fAp4:
